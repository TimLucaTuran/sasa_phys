...
def star_product_analyt(s_in1, s_in2, s_out, H, L):
    ...
    for h in range(H):
        for l in range(L):
            TF_1 = s_in1[h,l,0:2,0:2]
            TB_1 = s_in1[h,l,2:4,2:4]
            RF_1 = s_in1[h,l,2:4,0:2]
            RB_1 = s_in1[h,l,0:2,2:4]
            # S-matrix 2
            TF_2 = s_in2[h,l,0:2,0:2]
            TB_2 = s_in2[h,l,2:4,2:4]
            RF_2 = s_in2[h,l,2:4,0:2]
            RB_2 = s_in2[h,l,0:2,2:4]
            ...
            RF = RF_1 + TB_1 @ RF_2 @ left_kernel @ TF_1
            RB = RB_2 + TF_2 @ RB_1 @ right_kernel @ TB_2
            # Assemble the resulting s-matrix us_ing the elements from above
            s_out[h,l,0:2,0:2] = TF
            s_out[h,l,2:4,2:4] = TB
...
def star_product(s_in1, s_in2):
    """
    Setup for numba JIT compilation
    """
    H = max(s_in1.shape[0], s_in2.shape[0])
    L = s_in1.shape[1]
    s_out = np.zeros((H,L,4,4)).astype(np.complex)
    star_product_analyt(s_in1, s_in2, s_out, H, L)
    return s_out
