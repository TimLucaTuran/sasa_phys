function avg_square_final(PARGRID_processID, PARGRID_totalNumberOfProcesses, PARGRID_storageID)
...
Avg_Square = load('avg_square_shape_1-1_smallest.mat');
...
% Groove height in nm
Height_ = 55; % metal layer thickness in nm
Height_Ti = 4;
...
for loop = 1 : length(VAR_CELL{1})
    ...
    % refractive index of titanium (BB model)
    n_Ti = refractive_ind(lambda_, 13, 1);
    
    % switch thresholding type
    Layer_mask = Avg_Square.Layer_thresh_spec;

    STRUC{1} = (n_Au - n_embed) * Layer_mask + n_embed;
    STRUC{2} = (n_Ti - n_embed) * Layer_mask + n_embed;
